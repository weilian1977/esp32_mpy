...
STATIC mp_obj_t mpy_get_line_follower_value(mp_obj_t self_in, mp_obj_t ir_channel)
{
    uint8_t channel = mp_obj_get_int(ir_channel);
    int16_t ir_value = 0;
    if (channel == 1)
    {
        ir_value = sensor_value.ir1;
    }
    else if (channel == 2)
    {
        ir_value = sensor_value.ir2;
    }
    else if (channel == 3)
    {
        ir_value = sensor_value.ir3;
    }
    else if (channel == 4)
    {
        ir_value = sensor_value.ir4;
    }
    else if (channel == 5)
    {
        ir_value = sensor_value.ir5;
    }
    return mp_obj_new_float(ir_value);
}

STATIC MP_DEFINE_CONST_FUN_OBJ_2(get_line_follower_value_obj, mpy_get_line_follower_value);
...
STATIC const mp_rom_map_elem_t mpy_sensor_locals_dict_table[] =
{
    ...
    { MP_OBJ_NEW_QSTR(MP_QSTR_get_line_follower_value),        (mp_obj_t)&get_line_follower_value_obj },
