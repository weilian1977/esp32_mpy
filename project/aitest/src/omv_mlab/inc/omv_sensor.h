...
typedef struct __sensor omv_sensor_t;
typedef struct __sensor {
    ...
    omv_pixformat_t pixformat;      // Pixel format
    omv_framesize_t framesize;      // Frame size
    ...
    int  (*reset)               (omv_sensor_t *sensor);
    int  (*sleep)               (omv_sensor_t *sensor, int enable);
    int  (*read_reg)            (omv_sensor_t *sensor, uint16_t reg_addr);
    int  (*write_reg)           (omv_sensor_t *sensor, uint16_t reg_addr, uint16_t reg_data);
    int  (*set_pixformat)       (omv_sensor_t *sensor, omv_pixformat_t pixformat);
    int  (*set_framesize)       (omv_sensor_t *sensor, omv_framesize_t framesize);
    int  (*set_framerate)       (omv_sensor_t *sensor, int framerate);
    int  (*set_contrast)        (omv_sensor_t *sensor, int level);
    int  (*set_brightness)      (omv_sensor_t *sensor, int level);
    int  (*set_saturation)      (omv_sensor_t *sensor, int level);
    int  (*set_gainceiling)     (omv_sensor_t *sensor, gainceiling_t gainceiling);
    int  (*set_quality)         (omv_sensor_t *sensor, int quality);
    int  (*set_colorbar)        (omv_sensor_t *sensor, int enable);
    int  (*set_auto_gain)       (omv_sensor_t *sensor, int enable, float gain_db, float gain_db_ceiling);
    int  (*get_gain_db)         (omv_sensor_t *sensor, float *gain_db);
    int  (*set_auto_exposure)   (omv_sensor_t *sensor, int enable, int exposure_us);
    int  (*get_exposure_us)     (omv_sensor_t *sensor, int *exposure_us);
    int  (*set_auto_whitebal)   (omv_sensor_t *sensor, int enable, float r_gain_db, float g_gain_db, float b_gain_db);
    int  (*get_rgb_gain_db)     (omv_sensor_t *sensor, float *r_gain_db, float *g_gain_db, float *b_gain_db);
    int  (*set_hmirror)         (omv_sensor_t *sensor, int enable);
    int  (*set_vflip)           (omv_sensor_t *sensor, int enable);
    int  (*set_special_effect)  (omv_sensor_t *sensor, sde_t sde);
    int  (*set_lens_correction) (omv_sensor_t *sensor, int enable, int radi, int coef);
    int  (*ioctl)               (omv_sensor_t *sensor, int request, va_list ap);
    int  (*snapshot)            (omv_sensor_t *sensor, image_t *image, uint32_t flags);
} omv_sensor_t;

// Resolution table
extern const int omv_resolution[][2];
