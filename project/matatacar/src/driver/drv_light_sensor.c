...
static uint32_t light_sensor_2_get_value_and_filter(void)
{
    ...
    for(uint8_t i = 0; i < 10; i++)
    {
        adc2_get_raw(LIGHT_SENSOR_CHANNEL_2, ADC_WIDTH_12Bit, &adc_data);
        sum += adc_data;
        if(adc_data > val_max)
        {
            val_max = adc_data;
...
void light_sensor_init(void)
{
    ...
    adc2_config_channel_atten(LIGHT_SENSOR_CHANNEL_2, ADC_ATTEN_11db);
    //Characterize ADC
    val_type = esp_adc_cal_characterize(ADC_UNIT_2, ADC_ATTEN_11db, ADC_WIDTH_BIT_12, DEFAULT_VREF, &adc_chars);
    (void)val_type;
