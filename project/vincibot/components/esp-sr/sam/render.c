...
// contains the final soundsam_buffer
extern int sam_bufferpos;
extern char *sam_buffer;
extern int sam_bufferlength;
extern int sam_buffersize;
...
extern int speed_say_en_callback(int index,unsigned value);
int Output8BitAry(int index, unsigned char ary[5])
{
    int k;
    sam_bufferpos += timetable[oldtimetableindex][index];
    oldtimetableindex = index;
    for(k=0; k<5; k++) 
    {
       // printf("sam_buffer[%d]=%d\n",(sam_bufferpos/50+ k),ary[k]);
        //if(SAM_BUFFER_SIZE>(sam_bufferpos/50 + k))sam_buffer[sam_bufferpos/50 + k] = ary[k];
        //if(speed_say_en_callback(sam_bufferpos/50 + k,ary[k]))return AUDIO_BREAK;
        sam_bufferlength=sam_bufferpos/50 + k;
        if(sam_bufferlength<sam_buffersize)
        {
            sam_buffer[sam_bufferlength]=ary[k];
        }
        
    }
    return AUDIO_OK;
}
int Output8Bit(int index, unsigned char A)
{
    unsigned char ary[5] = {A,A,A,A,A};
    return Output8BitAry(index, ary);
}
...
int Render()
{
    ...
    int i;
    if (phonemeIndexOutput[0] == 255) return AUDIO_FAIL; //exit if no data
...
// pitch level (monotone).
    ...
    if (!singmode)
    {
        // iterate through the sam_buffer
        for(i=0; i<256; i++) {
...
// reset at the beginning of each glottal pulse.
    ...
    while(1)
    {
        ...
        if(A != 0)
        {
            ...
            // skip ahead two in the phoneme sam_buffer
            Y += 2;
            mem48 -= 2;
        } else
        {
            ...
            if(Output8BitAry(0, ary)==AUDIO_BREAK)return AUDIO_BREAK;
            speedcounter--;
    ...
    do
    {
        ...
        do
        {
            ...
            if ((tempA & 128) != 0)
            {
                ...
                // mem[54296] = X;
                sam_bufferpos += 150;
                printf("sam_buffer[sam_bufferpos/50]=%d\n",sam_bufferpos/50);
                //if(SAM_BUFFER_SIZE>(sam_bufferpos/50))sam_buffer[sam_bufferpos/50] = (X & 15)*16;
            } else
            {
                //mem[54296] = 6;
                X=6;
...
//pos48376:
    ...
    // backup 30 frames
    A = A - 30;
    // if index is before sam_buffer, point to start of sam_buffer
    if (Atemp <= 30) A=0;
    X = A;
