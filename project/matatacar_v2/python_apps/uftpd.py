...
#        self.logged_in = False
    ...
    def save_file_data(self, path, data_client, mode):
        ...
        with open(path, mode) as file:
            ...
            while bytes_read > 0:
                ...
                file_len = file_len + bytes_read
                bytes_read = data_client.readinto(buffer)
            data_client.close()
            print("\r\nsave file[%s] size:[%d]" %(path, file_len))
    ...
    def exec_ftp_command(self, cl):
        ...
        try:
            ...
            if err in (errno.ECONNABORTED, errno.ENOTCONN):
                close_client(cl)
...
def connect_wifi(ssid, password):
    ...
    print("ssid:%s, passwd:%s" %(ota_ssid, ota_password))
    print("connecting ...")
    wlan.config('mac')     
    wlan.ifconfig()       
